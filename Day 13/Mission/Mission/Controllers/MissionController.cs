...
namespace Mission.Controllers
{
    ...
    public class MissionController(IMissionService missionService) : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteMissionApplication(int missionApplicationId)
        {
            var result = await _missionService.MissionApplicationDelete(missionApplicationId);
            if (!result)
            {
                return NotFound();
            }
            return Ok(new { message = "Mission application disapproved successfully." });
        }
    }
}
