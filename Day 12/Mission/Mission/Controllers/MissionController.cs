...
namespace Mission.Controllers
{
    ...
    public class MissionController(IMissionService missionService) : ControllerBase
    {
        ...
        [HttpPost("MissionApplicationDelete")]
        public async Task<IActionResult> MissionApplicationDelete([FromQuery] int applicationId)
        {
            // For example, treat a "delete" as disapproval
            bool deleted = await _missionService.MissionApplicationDisapprove(applicationId);
            if (deleted)
            {
                return Ok(new { success = true, message = "Mission application deleted successfully." });
            }
